    ...
    <div>
      ...
      <div className={sharedStyles.wrapper}>
        ...
        <div className={sharedStyles.inputWrapper}>
          ...
          <Input className={sharedStyles.inputBorder} maxLength={10} onChange={(e) => setIntroduction(e.target.value)} />
        </div>
        <div className={sharedStyles.inputWrapper} style={{ alignItems: 'flex-start' }}>
          ...
          <Input.TextArea className={styles.textInput} cols={4} maxLength={50} onChange={(e) => setRole(e.target.value)} />
        </div>
        <div className={sharedStyles.inputWrapper}>
          <span className={sharedStyles.name}>志愿时长</span>
          <Input className={sharedStyles.inputBorder} onChange={(e) => setHour(e.target.value)} />
        </div>
        <div className={sharedStyles.inputWrapper}>
        ...
        <Button
          ...
          onClick={() => {
            if (date && name.length && introduction.length && role.length && hour.length && num) {
              dispatch!(createAddVolunteerActivity({
                name,
                introduction,
                role,
                hour,
